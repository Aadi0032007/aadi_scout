...
This recorder is designed to be used with teleop.py's SessionAndStreamManager:

    unlock -> recorder.start()
    lock   -> recorder.stop()

Important:
    stop() is robust and idempotent.
    If ffmpeg dies and _active becomes False, stop() will still finalize
    JSONL/session.json as long as a session folder exists.
"""
...
class SessionRecorder:
    ...
    def start(self) -> bool:
        ...
        Idempotent:
            ...
            - If an old inactive session is still open, stop() finalizes it first.
        """
        if self._active:
            return True
    ...
    def stop(self) -> None:
        ...
        Robust:
            - Works even if _active is already False.
            - Works after ffmpeg pipe failure.
        ...
        # Reset session state so next unlock creates a fresh session.
    ...
    def set_robot_lock(self, locked: bool) -> None:
        ...
        If True while active, tick loop skips frames.
        In this teleop design, lock normally calls recorder.stop(), but this
        remains useful during transitions/shutdown.
        """
    ...
    def _probe_and_start_ffmpeg(self, video_path: Path) -> Optional[str]:
        """Try each encoder in order. Return the first one that starts."""
        for encoder in self._encoder_pref:
            ...
            try:
                ...
                # Let ffmpeg fail fast if encoder is unavailable.
                time.sleep(0.4)
    ...
    def _tick_loop(self) -> None:
        ...
        while not self._stop.is_set():
            ...
            # If badly behind, resync instead of catching up aggressively.
            now = time.monotonic()
